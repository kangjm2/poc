...
- **레퍼런스 대비 워크벤치**(2026-09-02 갱신): ①`SOURCE_SAMPLE`(위경도·속도·서빙 PCI)과
  ...
  남은 것은 ⓪ **그래프의 두 번째 결과 모양** — 지금은 *표본당 한 행 × 출력 열 하나*로
  고정돼 있어 상태 점유 구간(`start_time`·`time_interval`)을 담을 곳이 없습니다 — 과
  그 위에 올라가는 ③ **진짜 State Machine** / **Previous·Current·Next** 입니다.
