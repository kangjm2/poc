...
│         ├── geom/      페인·경로의 기하 — 순수, DOM 없음, Node가   │
│         │              읽음. 화면과 문서가 같은 것을 부릅니다       │
│         └── doc/       워크북 문서와 페인 SVG(순수) + 가져오는 껍데기│
...
목록이 되며, `verify-scenarios` S20이 그 목록을 읽어 **면제마다 이유가 붙어 있는지**를 보고,
화면이 실제로 보낸 요청을 목록으로 **분류**합니다.

> *2026-09-04 정정 — 이 문단은 "목록을 읽어 각 엔드포인트를 두 번 호출한다"고 적었는데,
> 그렇지 않습니다.* 두 번 부르는 집합은 S20 안에 **손으로 적힌 열세 개 질의**이고, 목록은
> 이유를 검사하고 요청을 분류하는 데만 쓰입니다. 차이가 중요한 이유는 **목록에 이름을 올리는
> 것이 검사를 받는 것이 아니기 때문**입니다 — `serving-lines`는 만들어진 날 지킴으로 등재됐고
> 그날 조건을 걸어 부르는 검사가 하나도 없었습니다(같은 날 이 문서를 감사하다 찾아 고쳤습니다).
> 목록에 있는데 손으로 적은 집합에 없으면 아무도 모릅니다. `/api/cohorts`는 의도적으로 예외로,
> 주행이 여럿인 화면이라 S23이 자기 자리에서 좁아짐을 확인합니다.
>
> 어느 쪽도 없는 경로 — 지킴도 면제도 아닌 세션 분석 — 는 `api-surface.mjs`가 매핑에 대고
> 재서 잡습니다. 2026-09-04에 셋을 그렇게 찾았습니다.
...
면제된 **열네** 엔드포인트는 **목록에 이유와 함께** 실려 화면에서 읽힙니다. 지키지 못하는 것이
...
- **없는 세션** → 응답에 손대기 전에 **측정을 먼저 조회하는** 엔드포인트만 **404**입니다:
  `GET /sessions/{id}` · `/snapshot` · `/export.csv` · `/export.geojson` · `/report.html` ·
  `/field-to-lab` · `POST /field-to-lab/generate` · `DELETE /sessions/{id}` · `/compare`.
  *2026-09-04: 두 export가 여기 들어왔습니다* — 예전에는 헤더를 먼저 세워 없는 id에도
  **200에 그럴듯한 빈 파일**을 내줬고, 열 여섯 개에 행이 없는 CSV는 "존재하지 않는 주행"이
  아니라 "아무것도 기록하지 못한 주행"으로 읽힙니다. 판정 기준은 경로 이름이 아니라
  `analysis.getSession(id)` 호출입니다. 나머지 세션 범위 엔드포인트는 **200에 빈 결과**를 돌려줍니다 — 분석 쿼리가
...
| `scripts/verify-scenarios.mjs` | 여정 회귀 — 단계 간 상태가 이어짐 | 299단계 / 30 시나리오 |
| `tools/uxtest/api-surface.mjs` | **로직은 있는데 뷰가 없는** 격차 | 엔드포인트 · 클라이언트 · KPI 도달성 |
| `mvn test` | SQL을 조립하는 코드와 기하 | 99개 단위 테스트 |

단위 테스트가 **일곱 곳**에만 있는 것은 의도적입니다. 2026-09-03에 둘이 늘었습니다 —
`CohortTest`(13)는 그룹의 백분위가 세션별 요약에서 복원되지 않기 때문이고(그래서 풀링 질의의
모양 자체가 검사 대상입니다), `ComputedKpisTest`(6)는 `KpiGraph`와 같은 이유입니다.
2026-09-04에 `ExportFormatTest`(8)가 늘었습니다 — 내보내기 파일의 **서식은 씨드 주행이 목격할
수 없기 때문**입니다: 시드의 세션 이름에는 쉼표가 없고, 값은 지수 표기로 넘어가지 않으며,
좌표는 자릿수가 짧습니다. 브라우저로 전부 내보내도 작성기에 대해서는 아무것도 증명되지
않습니다(§1.5.12).
...
| DB 마이그레이션 | Flyway, 기동 시 자동 (`V1` 스키마 → `V2` 파티셔닝·랩 도메인 → `V3` 미사용 테이블 제거 → … → `V10` 이벤트 타입 레지스트리 → `V11` 임포트 취소 → `V12` 색상셋 종류·이벤트 색 재정의·레이어의 측정 → `V13` Classifier 개명 + 그래프 문서 버전 → `V14` 핸드오버 원인 이벤트 타입(`MISSING_HANDOVER`·`MISSING_NEIGHBOUR`, DERIVED) 등록) |
