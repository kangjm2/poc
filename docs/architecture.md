...
│  그 밖의 도메인 테이블 25개 (측정·KPI 카탈로그·랩·워크북·임포트)     │
└────────────────────────────────────────────────────────────────────┘
```
...
### 4.5a 시점을 정하는 입력은 좌표가 아니라 이름으로 지목합니다

레퍼런스의 상관 노드 계열은 *"가장 왼쪽 입력이 primary"* 라는 규칙 위에 서 있고, primary가
**언제 출력이 존재하는지**를 정합니다(p354). "가장 왼쪽"은 x 좌표인데, 이 컴파일러는 노드
좌표를 읽지 않습니다 — 같은 그림이 항상 같은 SQL이 되도록 **입력을 노드 번호로 정렬**하기
때문입니다(그 정렬이 없던 시절 똑같아 보이는 두 캔버스가 서로 다른 KPI를 냈습니다).

그래서 `CORRELATE`의 primary는 **필드**입니다. 배치 규약을 배우는 것보다 어느 입력이
시점을 정하는지 적힌 셀렉트 하나가 작습니다.

두 가지가 이 노드의 정확성을 집니다.

- **창은 두 입력의 합집합 위에서 돕니다.** "이 이벤트 직전의 값"은 primary가 행을 갖지 않는
  표본까지 볼 수 있어야 하고, 이벤트 소스라면 그게 거의 전부입니다.
- **게이트는 맨 마지막에 겁니다.** 먼저 걸면 "직전"이 *"직전 이벤트의 값"* 이 됩니다 —
  다른 질문에 대한, 완벽하게 그럴듯한 답입니다.

`Within (ms)`는 레퍼런스에 없는 우리 추가입니다. 경계가 없으면 "직전"이 주행 전체를 거슬러
20분 전 값으로 답할 수 있습니다.

### 4.6 구간은 새 테이블이 아니라 행 하나에 들어갑니다

레퍼런스 State Machine의 출력은 `start_time | end_time | time_interval` 세 열이지만
`end_time = start_time + time_interval`이라 **자유도가 둘**이고, `sample_kpi` 행에도 둘이
있습니다(`ts`, `value`). 그래서 머문 시간을 **진입한 표본에** 찍습니다 — `ts`가 곧
`start_time`입니다.

이것이 두 번째 결과 모양도, 새 테이블도, `recompute()`의 분기도 없는 이유이고, 동시에
구간이 **이미 있는 모든 경로**(색칠·비닝·전역 필터·내보내기·리포트)를 그대로 타는 이유입니다.
전역 필터의 면제 목록(§4.6.1)에 항목이 하나도 늘지 않은 것도 같은 까닭입니다 — 구간 행은
표본 행입니다.

기억은 **사다리**입니다: 상태 k는 k−1에서만 진입하고, 첫 상태의 조건이 유일한 복귀입니다.
레퍼런스의 기억은 두 가지 일을 하는데(상태 순서 강제 + 스트림이 흩어 놓은 파라미터 값
모으기) 우리에게 필요한 것은 첫째뿐입니다 — 둘째는 `(session_id, seq)` 스키마가 이미
해결했습니다. 임의 전이표는 순차 접기라 `WITH RECURSIVE`가 필요하고, 그 질의를 매 임포트마다
전 주행에 돌리게 됩니다. **하지 않기로 했고, 화면이 그렇게 적습니다.**

그리고 한 줄이 이 기능 전체의 정직성을 집니다: 구간은 그 **안에서 측정이 끊긴 적이 없을
때만** 발행됩니다. 표본별 끊김 카운터(`RouteContinuity`)를 주행의 `sample`에서 읽어 진입과
종료의 값이 같은지 봅니다 — 발행하려는 구간의 **내용**을 재는 것이지 대리 신호를 보는 것이
아니고, 노드의 행이 표본마다든 5분에 하나든 똑같이 동작합니다.

### 4.6.1 전역 필터는 절이 하나여야 검사할 수 있다
...
면제된 **열** 엔드포인트는 **목록에 이유와 함께** 실려 화면에서 읽힙니다. 지키지 못하는 것이
문제가 아니라 **지키지 못한다는 사실이 보이지 않는 것**이 문제이기 때문입니다.

**절이 세션 하나에 매이지 않습니다.** `scope()`가 내놓는 키는 `(alias.session_id, alias.seq)`
행값이고, 여러 주행을 받으면 각 주행의 선택을 **더합니다**. 그래서 코호트 화면(§4.7)이 조건을
그대로 지킵니다 — 그리고 `spatial-diff`·`compare`의 면제 사유가 "한 주행의 표본만 지목할 수
있다"에서 **"필터 파라미터를 아직 받지 않는다"**로 바뀌었습니다. 사유가 사실이 아니게 된
목록은 목록이 막으려던 바로 그 결함이므로 같은 커밋에서 고쳤습니다.

### 4.7 그룹의 중앙값은 멤버들의 중앙값에서 나오지 않습니다

코호트 화면은 주행 속성(빌드·시나리오·단말·사업자·기술·장소)으로 주행을 묶어 그룹마다
통계 하나를 냅니다. 설계에서 정한 것은 두 가지입니다.

**저장된 폴더가 아니라 차원값으로 묶습니다.** `measurement_folder` 테이블은
`measurement_session`이 이미 아는 사실의 두 번째 집이 되고, 주행 삭제 시의 수선 규칙이
하필 `view/state.ts`에 생기며, 코호트 링크가 세션 id를 실어야 합니다 — 그 파일이 존재하는
이유가 정확히 그것을 조심하기 위해서입니다. 차원값은 셋 다 없습니다.

**한 번의 풀링 질의이고 N개의 합성이 아닙니다.** 개수·최소·최대·가중합은 세션별 부분합에서
정확히 합쳐지지만 **백분위는 안 됩니다**. 클라이언트에서 세션별 `Statistics`를 합쳤다면 맞는
평균 옆에 없거나 지어낸 CDF를 실었을 것입니다. 그래서
`WeightedStats.computeAcross(SessionSet, …)`가 진짜 구현이고 한 세션짜리가 그리로 위임합니다 —
이 응용에 평균은 여전히 **하나**입니다. 조인 키도 `(session_id, seq)` 쌍이라야 합니다: `seq`는
주행마다 처음부터 다시 세므로 `seq`만으로 조인하면 주행 수만큼 행이 늘어납니다.

**거리 가중은 여러 주행에 걸쳐 거부합니다.** 로거가 끊긴 뒤의 표본이 측정되지 않은 구간
전체의 무게를 지므로, 한 주행 안에서는 공정한 거래가 두 주행 사이에서는 **로거가 더 자주
끊긴 쪽이 이기는** 편향이 됩니다.

**교란 가드.** 두 번째 차원을 고정하면 모든 버킷에 나타나는 값의 주행만 남고, 빠진 것은 전부
**이름과 이유로** 나열됩니다. 어느 버킷에도 공통값이 없으면 빈 차트가 아니라 **거부**이고,
거부문이 그룹별로 무엇을 달렸는지 적습니다. 고정 차원이 없으면 델타는 있고 **판정은
없습니다** — 가드 없는 "낫다"는 빌드만큼이나 도로를 잽니다.

---

## 5. API

분석·KPI·랩·임포트 경로입니다. **전부는 아닙니다** — 워크북 3개, KPI 그래프 5개,
`event-types`, `field-to-lab`, `problem-survey`, 랩 상세 조회는 이 표에 없습니다. 전체 목록은
문서가 아니라 `node tools/uxtest/api-surface.mjs`가 소스에서 세어 출력합니다. *(2026-09-03
정정: 이 문단은 "`/api` 하위 전체입니다"로 시작했고, 그때도 스무 개쯤 빠져 있었습니다.)*
...
| `scripts/verify-scenarios.mjs` | 여정 회귀 — 단계 간 상태가 이어짐 | 264단계 / 26 시나리오 |
...
모양 자체가 검사 대상입니다), `ComputedKpisTest`(6)는 `KpiGraph`와 같은 이유입니다.
원래 문장: 나머지 코드는 값을 파라미터로
...
| DB 마이그레이션 | Flyway, 기동 시 자동 (`V1` 스키마 → `V2` 파티셔닝·랩 도메인 → `V3` 미사용 테이블 제거 → … → `V10` 이벤트 타입 레지스트리 → `V11` 임포트 취소 → `V12` 색상셋 종류·이벤트 색 재정의·레이어의 측정 → `V13` Classifier 개명 + 그래프 문서 버전) |
...
| 랩 런 **실물 장비 연동** | 스텁 설계가 선행되어야 함. **브링업 시퀀스와 판정은 구현됐고**(⑨), 남은 것은 실제 장비에 명령을 내리는 부분입니다 |
| 사전 집계 테이블 | `kpi_rollup`을 만들었다가 **제거**했습니다. 200 device-hours 실측에서 직접 집계가 충분했고, 쓰이지 않는 스키마는 오해만 남깁니다 |

> **2026-09-02 정정.** 이 표에 **파생 KPI 수식**이 남아 있었습니다. 그 뒤에 구현됐고 표만
> 뒤처져 있었습니다. *2026-09-03 재정정: 같은 문장이 **랩 런 실행기**도 구현됐다고 적었는데
> 그것은 틀렸습니다 — 실행기는 코드에 없고, 위 표의 "랩 런 실물 장비 연동" 줄이 지금도
> 맞습니다. 있는 것은 브링업 시퀀스와 판정이고 없는 것은 장비에 명령을 내리는 부분이라,
> 그 줄이 이미 정확히 그렇게 적고 있습니다. 낡은 목록을 고치다 반대 방향으로 틀린 셈입니다.* 같은 날 `README.md`의
> "알려진 제약"에서도 `SOURCE_SAMPLE`·`SOURCE_EVENT`를 미구현으로 적어 둔 줄을 고쳤습니다.
> **하지 않기로 한 것의 목록은 구현 목록보다 늙기 쉽습니다** — 무언가를 만들 때 지우는
> 것을 잊기 쉽고, 잊어도 아무 검사가 실패하지 않기 때문입니다.
