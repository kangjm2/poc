...
| **지도 Layers 목록** (UC13, p150) | 중 — *2026-09-02 신규.* 레퍼런스는 지도 우측에 `Tools` 아이콘 행 + **레이어 목록**(레이어별 색 아이콘·접기)을 둡니다. 우리 지도 오버레이 9종(경로·이벤트·셀 마커·푸트프린트·모니터드셋 선·영역 빈·거리 빈·폴리곤·공간 차분)은 툴바 셀렉트와 on/off 버튼으로 **흩어져** 조작되고, 지금 무엇이 그려지고 있는지 한자리에서 보여주는 목록이 없습니다 |
| **MAP 페인이 자기 레이어를 읽지 않음** | 작음 — *2026-09-02 신규, 결함.* `ComposedWorkbook.tsx:202`가 `visible[0]`을 **캡션에만** 씁니다. RouteMap에는 오버레이 prop이 하나도 넘어가지 않아 두 번째·세 번째 레이어를 체크해도 화면이 그대로입니다. 차트 페인만 `layers`를 실제로 소비합니다 |
...
| **P2-1** | 빈을 **어느 통계로 칠할지** | 매우 작음 | `minValue`·`maxValue`·`sampleCount`가 **이미 응답에 있습니다** |
| **P2-2** | **푸트프린트 필터**(PCI 목록) | 작음 | 셀 수십 개에서 좁힐 수단이 하나도 없습니다 |
| **P2-3** | **폴리곤을 필터로** | 작음 | 도형과 `AreaSelection.inside`가 이미 있고, 술어를 색칠 경로에 걸면 됩니다 |
| **P2-4** | 푸트프린트 **3강 기준** · 거리 빈 **40λ** | 작음 | 데이터가 이미 있는 기존 두 항목. P2-2와 같은 화면이라 함께 |
| **P3-1** | **지도 Layers 목록** | 중 | 오버레이 9종이 흩어져 있고 무엇이 켜졌는지 볼 곳이 없습니다 |
| **P3-2** | **전역 필터**(셀 ID · 이벤트 제외 · 폴리곤이 첫 입력) | 중 | UC5의 실제 진입점 셋이 사양으로 확보됐습니다 |
| **P3-3** | 색상셋 **gradient · string 타입** + 이름 붙은 재사용 | 중 | UC29–31 셋이 여기 매여 있습니다 |
| **P3-4** | 공간 차분 **측정 그룹** · 레이어 = 주행 | 중 | 같은 뿌리 — 화면이 주행 하나만 다룹니다 |
| **P4-1** | **State Machine 구간 출력**(`start_time`·`time_interval`) | 큼 | 저장 모델을 건드리는 유일한 항목. UC27이 여기 걸립니다 |
| **P4-2** | **Previous / Current / Next** 상관 | 중 | 근본 원인 분석의 핵심 질문. `<Previous value>`가 레퍼런스 사양 |
| **P4-3** | **세션 간 집계 모델** | 큼 | 대시보드 · 추세 · 벤치마킹 리포트 · NPS가 전부 이것 하나에 매여 있습니다 |
