...
| ~~P1~~ | ~~KPI Workbench — **노드 그래프** 빌더~~ | **완료** (`Import` 화면). 레퍼런스 워크벤치 스크린샷을 직접 판독해 설계했고 — **그 "스크린샷"의 정체가 이번에 밝혀졌습니다: 매뉴얼 p405의 UC27 상태 흐름도였습니다**(파일 크기가 정확히 일치) — 결정적 단서는 Output 노드의 `Column count: 19`였습니다 — 그래프는 **값이 아니라 이름 붙은 열을 가진 행 집합**의 데이터플로라는 뜻이고, 그래서 수식으로는 표현할 수 없습니다. 정렬 노드는 의도적으로 없습니다 |
