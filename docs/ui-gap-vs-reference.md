...
| **P3-2** | **전역 필터** | **큼** | *재사정.* 표본을 읽는 자리 57곳 · 엔드포인트 43개. 일부에만 걸면 "모든 조작"이 거짓말이 됩니다 |
| **P3-3** | 색상셋 **gradient · string 타입** | 중 | 아직. 이름 붙은 재사용은 **별도 큼 항목**으로 분리 |
| ~~**P3-4**~~ | 공간 차분 **측정 그룹** | 중 | ✅ 완료. 레이어 = 주행은 **별도 큼 항목**으로 분리 |
| **P3-5** | 레이어 = **주행** | **큼** | *분리.* 레이어가 KPI 하나이고 트랙 조회가 단일 세션이라, 화면이 아니라 데이터 흐름의 변경 |
