...
| `Missing neighbour` 원인 분류 (§4) | **설정된** 이웃 목록. 측정된 이웃과 다른 것이며 우리 데이터에 없음 | ⛔ 여전히 막힘 |
| `5G NR Beams` 탭 (§3) | SSB 빔 인덱스별 measurement | ⛔ |
| 영상 동기화 재생 (§7) | 주행 영상 소스 자체가 없음 | ⛔ |

남은 셋은 여전히 화면을 그리는 문제가 아니라 **측정에 없는 값을 어디서 가져올 것인가**의
문제입니다. 특히 `Missing neighbour`는 이웃 셀을 측정해도 풀리지 않습니다: "빠진 이웃"은
*측정된* 이웃과 *설정된* 이웃 목록의 차집합인데, 우리는 후자를 갖고 있지 않습니다.
이 구분을 하지 않으면 V7으로 닫혔다고 잘못 적게 됩니다.
...
| **워크벤치 State machine이 상태를 기억하지 않음** | 중 — 표본별 `CASE`라 래칭(히스테리시스)이 없습니다. 이름이 실제보다 큽니다 |
...
| ~~P1~~ | ~~KPI Workbench — **노드 그래프** 빌더~~ | **완료** (`Import` 화면). 레퍼런스 워크벤치 스크린샷을 직접 판독해 설계했고, 결정적 단서는 Output 노드의 `Column count: 19`였습니다 — 그래프는 **값이 아니라 이름 붙은 열을 가진 행 집합**의 데이터플로라는 뜻이고, 그래서 수식으로는 표현할 수 없습니다. 정렬 노드는 의도적으로 없습니다 |
