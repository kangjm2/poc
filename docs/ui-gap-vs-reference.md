...
| ~~**P3-2**~~ | **전역 필터** | **큼** | ✅ 완료. **12개 엔드포인트가 지키고 8개가 면제**이며, 면제는 목록에 이유와 함께 실려 화면에서 읽힙니다. 아래 (b-2) 참조 |
...
**2026-09-02 현재 P0·P1·P2·P3가 모두 닫혔고, P4-1도 닫혔습니다.**
...
그 목록이며, 화면의 **Reach: 12 of 20** 버튼이 같은 목록을 펼칩니다.

| 지킴 (12) | 면제 (8) · 이유 |
|---|---|
| `track` · `series` · `distribution` · `statistics` · `cell-breakdown` · `degradations` · `area-statistics` · `bins` · `cell-footprints` · `export.csv` · `export.geojson` · `report.html` | `events` · `messages` — **시각(ts)으로 매인 행**이라 표본 필터가 고르지 못합니다 |
| | `snapshot` — 표본 하나를 번호로 지목 |
| | `monitored-set` · `pilot-pollution` · `neighbour-breakdown` — 이웃 행 / 구간 판정. 가운데 표본을 빼면 **한 구간이 두 개로 보고**됩니다 |
| | `spatial-diff` · `compare` — 둘 이상의 주행. 필터는 한 주행의 표본을 지목합니다 |

구현은 **`seq IN (...)` 하위질의 한 줄**입니다. 이 스키마의 모든 분석이 세션 하나 안에서 `seq`로
