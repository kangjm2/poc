...
**무엇이 조건을 지키지 않는지도 화면이 말합니다.** 오른쪽 **Reach: 15 of 29**를 누르면
지키는 15개와 **지키지 못하는 14개를 이유와 함께** 폅니다. 이벤트와 L3 메시지는 표본이
아니라 **시각**으로 매인 행이고, 모니터드 셋과 파일럿 오염은 구간 판정이라 가운데 표본을
빼면 한 구간이 둘로 보고됩니다. 공간 차분과 비교는 **아직 필터 파라미터를 받지 않습니다** — 조건이 `(session_id, seq)` 키
집합이 된 뒤로 주행이 여럿인 것은 더 이상 이유가 아니고, 배선이 안 됐을 뿐입니다. 화면의
목록이 그렇게 적습니다.
