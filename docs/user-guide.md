...
| | |
|---|---|
| 범례 제목 | `RSRP (NR SpCell) (dBm) **[Sample]**` — 마지막 대괄호는 비율의 **가중 기준**입니다. 참조 도구가 `[Time]`이라 적는 자리입니다. 기본은 표본 가중이고, **거리 가중으로 바꾸면 표기도 `[Distance]`로 함께 바뀝니다** — 이름과 계산이 어긋날 자리를 두지 않았습니다 (§3.6c) |
...
**원인은 여러 개를 동시에 열어 둘 수 있습니다.** 두 번째 조각을 클릭하면 파이 아래에
**탭이 하나 더** 생기고, 탭의 색은 그 조각의 색입니다. 탭을 오가면 아래 사례 표가
그때그때 바뀝니다 — RACH 실패와 핸드오버 실패를 나란히 놓고 비교하는 것이 이 화면의
실제 사용법이고, 슬롯이 하나면 그 비교를 **기억으로** 하게 됩니다.

> **지금 읽고 있는** 원인의 조각을 다시 클릭하면 그 탭이 닫힙니다 — 전부로 돌아가는 가장
> 짧은 길입니다. 탭의 `×` 로 닫으면 **남아 있는 탭으로 물러납니다**: 닫은 것은 탭
> 하나이지 하던 비교 전체가 아닙니다.
...
| **Missing handover** · **Missing neighbour** | 이동성 검출기 — 아래 |

> 무선 KPI의 열화는 **일부러 빠져 있습니다.** 커버리지 검출기가 이미 같은 구간을 분류하고
> 있어서, 둘 다 세면 한 구간이 두 원인으로 중복 집계됩니다.
>
**이동성 두 원인은 표본을 보고 판정합니다.** 다른 셀이 **3 dB 이상 더 강한 채로**
**네 표본 이상** 이어지는데도 서빙이 바뀌지 않은 구간입니다(**여덟 표본 이상**이면 CRITICAL). 둘을 가르는 것은 **그 더 강한 셀이 이
주행에서 한 번이라도 서빙한 적이 있는가**입니다 —

- 있으면 **Missing handover**: 갈 수 있는 셀인데 안 갔습니다. 문턱이나 타이밍 문제입니다.
- 없으면 **Missing neighbour**: 단말이 **보고 있는데 갈 수 없는** 셀입니다. 이웃 목록에
  없을 가능성이 높고, 고치는 사람도 고치는 방법도 다릅니다.

> 두 원인을 하나로 묶으면 "핸드오버가 안 됐다"까지만 알고 끝납니다. 우리가 나눈 이유는
> **그 둘의 조치가 다르기 때문**입니다.
...
| **Pilot pollution** | **몇 개**의 **쓸 만한** 셀이 **몇 dB** 안에 몰린 구간 — 둘 다 패널에서 고릅니다(기본 3개 · 6 dB). 행을 누르면 그 시각으로 이동 |
...
> 닿지 않는 것입니다.
>
> **두 숫자는 판정 그 자체입니다.** 창을 12 dB로 넓히고 개수를 2로 낮추면 같은 주행에서
> 구간이 한 자리에서 두 자리로 늘어납니다. 어느 쪽이 맞는지는 망의 설계에 달려 있으므로
> 도구가 정할 일이 아니고, 그래서 **묻습니다.** 패널 설명은 자기가 보낸 값을 되읽으므로,
> 화면에 적힌 조건은 항상 그 표를 만든 조건입니다.
...
헤더의 **Export document**가 워크북 전체를 **한 장짜리 HTML 문서**로 내려줍니다. 그 옆의
**pane 1 / pane 2 …** 는 페인 하나씩을 **홀로 열리는 SVG**로 내려줍니다.

- **PDF는 브라우저가 만듭니다.** 문서를 열고 인쇄하면 됩니다 — 인쇄 규칙이 들어 있어
  **페인마다 한 장**으로 끊깁니다. 파일 서두가 자기가 PDF·MS Word·MS PowerPoint가 **아니라고**
  적습니다. 우리가 그 셋을 직접 쓰지 않는 이유는 §3.8 마지막 인용과 같습니다.
- **문서는 화면과 같은 계산으로 그려집니다.** 두 번 그리지 않습니다 — 같은 모듈이 페인의
  경로와 눈금과 범례 숫자를 만들고, 화면은 그것을 SVG 요소로, 문서는 SVG 텍스트로 씁니다.
- **저장하지 않은 편집도 나갑니다.** 방금 추가한 페인이 문서에 들어 있고, 서두에
  `saved: no — exported from unsaved edits on the screen`이라고 적힙니다. 보고 있는 것을
  내보내는 것이 맞고, 그것이 저장된 것과 다르면 파일이 말합니다.
- **조건·측정이 서두와 페인마다 함께** 실립니다(§3.8의 이유 그대로 — 페인 하나를 슬라이드에
  끌어다 놓으면 서두는 따라가지 않습니다). 조건 문장은 서버가 준 문장 그대로입니다.
- **지도 페인에는 배경 지도가 없습니다.** 도로와 지명은 타일 서비스의 것이고 이 파일의
  일부가 아닙니다 — 파일이 그렇게 적고, 대신 **런(run)마다 한 줄짜리 표**가 붙습니다. 화면에서
  마우스를 올려야 보이던 시각·값·구간·서빙 PCI·표본 수가 그 표에 있습니다.
- 내보내는 단위는 **페인**입니다. 레퍼런스의 단위는 **페이지**인데(p220) 우리 워크북에는 아직
  페이지가 없어서, 없는 것을 있는 말로 부르지 않았습니다.
...
**양쪽 다 여러 주행일 수 있습니다.** 각 편의 `+ add to…` 로 측정을 더하면 그 편은
**그룹**이 되고, 배너가 그 편이 무엇으로 이루어졌는지 이름으로 적습니다. 그룹 평균은
**풀링**입니다 — 주행별 평균의 평균이 아니라, 그 그룹이 그 타일에 낸 표본 전체의 평균.
둘은 주행마다 표본 수가 다를 때 갈리고(정체를 기어간 주행이 표본이 많습니다), 풀링이
*"이 그룹이 여기서 뭘 쟀나"* 에 답합니다.

> 한 편에 주행을 더하면 **덮은 땅도 늘어납니다.** 시내 주행 하나만으로는 한쪽만 지난
> 타일이 1개인데, 고속 주행을 더하면 104개가 됩니다. 그래서 *"저녁 주행들 대 아침
> 주행들"* 같은 물음은 양쪽이 다 그룹일 때만 물어집니다.
>
> 같은 측정이 양쪽에 오면 **거부합니다** — 자기 자신과의 차분은 타일을 "일치함"으로
> 끌어당깁니다.

### 3.5g 차트에서 한 구간만 확대한다

차트 머리글의 **zoom** 을 누르면 `drag a span…` 으로 바뀝니다. 관심 구간을 가로로 끌면 그
구간만 남고, 머리글이 **몇 표본을 보고 있는지**와 **whole drive** 버튼으로 바뀝니다.

> **왜 모드인가.** 차트 위의 기본 드래그는 **커서 스크럽**이라, 확대를 그냥 드래그에
> 붙이면 매일 쓰는 제스처를 빼앗습니다. 켜져 있는 동안에는 머리글이 그렇게 적혀 있고,
> 돌아갈 길도 같은 자리에 있습니다.
>
> **부모가 이미 창을 정해 준 차트에는 나타나지 않습니다** — Problem Survey에서 사례를
> 눌러 연 맥락 창이 그렇습니다. 한 도메인에 주인이 둘이면 서로 싸웁니다.

**임계선.** KPI에 임계 사다리가 있으면 차트가 그 **경계**를 가로선으로 긋습니다. 사다리
전부가 아니라 **심각도가 바뀌는 자리**만입니다 — 선이 다섯이면 어느 것이 "나쁨의 시작"인지
읽을 수 없기 때문입니다. 확대해서 임계가 창 밖으로 나가면 `thresholds outside this range` 라고 적습니다 —
아무것도 그리지 않으면 *"이 차트에는 임계가 없다"* 로 읽히는데, 그것은 다른 사실입니다.
...
- **내보내기는 좁혀지지 않습니다** — 이 칩은 화면의 숫자를 좁히지, 파일을 좁히지 않습니다.
  파일에서 구간을 좁히고 싶으면 §3.6-1의 전역 필터를 쓰십시오. 그쪽은 파일에도 실립니다

칩의 ✕로 해제합니다.

![범위 필터가 걸린 통계](./poc-screenshots/18-range-filter-statistics.png)

### 3.6-1 조건 하나를 걸고 전부를 그 조건으로 본다 — 전역 필터

3.6의 구간 필터는 **시간**을 좁힙니다. 전역 필터는 **표본**을 좁힙니다 — 그리고 좁힌 상태로
화면 전체가 답합니다.

툴바 바로 아래 줄에 씁니다.

```
kpi:RSRQ:>=:-12     RSRQ가 -12 dB 이상인 표본만
cell:101            PCI 101이 서빙한 표본만
cell:101;kpi:RSRP:<:-100   둘 다 (세미콜론은 AND)
notevent:CALL_DROP  그 이벤트 근처(±5표본)를 뺀 나머지
```

**손으로 쓰지 않아도 됩니다.** 지도의 셀 사이트를 **우클릭**하면 `Filter to PCI 101` 버튼이
뜨고, 누르면 같은 조건이 줄에 들어갑니다 — 우클릭 자체로 걸지 않는 이유는 필터가 화면
전체를 다시 좁히기 때문입니다(붐비는 지도에서 사고로 발동할 조작입니다). 이벤트 목록의
행 끝에는 **exclude** 가 있습니다.

> **`notevent:` 의 ±5표본은 우리가 더한 것이고, 필터 설명이 그 숫자를 말합니다.** 매뉴얼이
> 지우는 것은 실패한 **호(call)** 인데 우리 데이터에는 호 경계가 없습니다. 그리고 표본 하나를
> 빼는 것은 어떤 백분위도 바꾸지 않습니다 — 실측으로 1200표본이 1178이 되고 **P05가
> −115.5에서 −114.1로** 올라갑니다. 나쁜 호 하나가 백분위에 앉아 있었다는 뜻이고, 그것이
> 이 기능의 요점입니다. 쓰는 자리는 **이벤트가 망 탓이 아니라고 판단할 때**입니다 — 단말을
> 껐거나, 시험 자체가 잘못 걸렸거나.

**Apply**를 누르면 줄이 노랗게 바뀌고 `In force: RSRQ >= -12`라고 적힙니다. 그 순간부터
경로 색, 범례, 통계, CDF, 셀별 막대, 열화 목록, 영역 빈, 푸트프린트, 그린 도형의 통계,
그리고 **CSV·GeoJSON·리포트**까지 같은 조건으로 답합니다. 리포트는 조건을 **머리글에도**
적습니다 — 화면에서 멀리 떨어져 읽히는 산출물이라, 조건을 적지 않으면 전체 주행의 표와
구별되지 않기 때문입니다.

**무엇이 조건을 지키지 않는지도 화면이 말합니다.** 오른쪽 **Reach: 17 of 29**를 누르면
지키는 17개와 **지키지 못하는 12개를 이유와 함께** 폅니다. 이벤트와 L3 메시지는 표본이
아니라 **시각**으로 매인 행이고, 모니터드 셋과 파일럿 오염은 구간 판정이라 가운데 표본을
빼면 한 구간이 둘로 보고됩니다. 공간 차분과 Problem Survey는 **아직 필터 파라미터를 받지
않습니다** — 조건이 `(session_id, seq)` 키 집합이 된 뒤로 주행이 여럿인 것은 더 이상 이유가
아니고, 배선이 안 됐을 뿐입니다. 화면의 목록이 그렇게 적습니다.

*2026-09-05 — 비교와 커버리지 이슈가 조건을 지킵니다.*

- **Compare**는 두 주행을 **각자 자기 자신에 대해** 좁혀 비교합니다. "RSRQ가 쓸 만하던 곳"은
  주행마다 다른 도로이고, A의 그 도로를 B의 주행 전체와 견주는 것은 아무 답도 아닙니다.
  머리글이 **기록한 수와 조건이 고른 수를 함께** 적습니다(`1174 / 1200 recorded · 558 / 674
  under this condition`) — 기록한 수는 조건이 움직이면 안 되는 값이라서입니다. 시드 주행에서는
  이것이 **판정을 뒤집습니다**: RSRP가 주행 전체로는 0.67 dB 나아지고, RSRQ가 쓸 만하던
  곳에서는 0.94 dB 나빠집니다.
- **Detected coverage issues**는 세 검출기 전부가 조건을 지킵니다. 좁히는 시점이 **구간을
  묶기 전**이라, 구간은 "나쁘고 **또한** 선택된" 표본의 연속입니다. 그래서 나쁜 구간 가운데
  표본이 빠지면 **한 구간이 둘로 갈라지고, 개수가 늘 수도 있습니다** — 패널이 그렇게 적습니다.

> 조건이 걸린 화면인지 아닌지는 **줄의 색과 문구**로 언제나 보입니다. 아무것도 없으면
> `No global filters`라고 적혀 있습니다. 조건을 걸어 두고 잊는 것이 이 기능의 유일한
> 위험이라, 그 줄은 접히지 않습니다.

주소창에도 실립니다(`gf=`). 링크를 받은 사람은 **같은 조건이 걸린 화면**을 엽니다 — 조건을
빼고 보내면 보낸 화면과 자기 화면이 다른데도 양쪽 다 앞뒤가 맞아 보입니다. 문법에 맞지 않는
조건은 **적용되지 않고** 거절 사유가 표시되며, 링크로 들어온 것이면 3.5b의 수정 알림에
`gf=…  → off`로 나옵니다.
...
**셀이 많으면 좁힙니다.** `Footprints` 옆 입력에 PCI를 적으면 그 셀만 그립니다 —
`3,10-30,42,100-` 형식으로, 낱개·범위·**열린 범위**를 섞어 쓸 수 있습니다. `100-` 은
"100번 이상 전부"라는 뜻이고, 어디서 끝나는지 모르는 채로 한 층을 가리키는 방법입니다.

> 지도 머리글이 **무엇을 뺐는지** 적습니다 — `footprints for 8, 21 (2 of 5 cells)`.
> 다섯 중 둘을 그린 지도는 다섯을 그린 지도와 **다른 그림**인데, 화면의 다른 어떤 것도
> 어느 쪽을 보고 있는지 알려주지 않습니다. 못 알아들을 것을 적으면 **말없이 따르지 않고**
> 같은 자리에 `cell filter ignored: …` 라고 적은 뒤 전부를 그립니다.

`Monitored Set` 탭의 **Across the whole drive** 표(Detected · Seen % · Contended · Served)와
함께 보면 오버슈팅 셀이 드러납니다 — 검출률 대비 서빙률은 그 표에 있습니다(§3.4a).
`Cells` 탭이 세는 것은 전부 그 셀이 **서빙한** 표본입니다.

### 3.6b-1 셀이 실제로 어디에 서 있나 — Cell locator

`Cells` 탭에는 지도가 있고, 그 위에 **기록된 셀 위치**와 **주행이 추정한 위치**가 함께
그려집니다 — 추정은 보라색이고 점선이 짝을 잇습니다. **표가 그 거리를 미터로 적습니다**:
지도 위의 선 길이는 읽을 축척이 없어서, 200 m는 그것이 두 블록 차이일 때에도 작아 보입니다.

**이 화면이 묻는 것은 "셀이 어디 있나"가 아니라 "기록이 아직 맞나"입니다.** 셀 데이터베이스는
이설·급전 교체·좌표 오기입으로 낡고, 드라이브는 그것에 대해 누구나 정기적으로 모으는 유일한
독립 증거입니다. 추정과 기록이 300 m 어긋나면 의심할 것은 **기록**입니다.

**min score** 로 걸러 봅니다(0–10). **6 이상은 100 m 이내**를 뜻하고, 그 주장은 검사가
겁니다 — 우리 검사는 공식이 아니라 이 문장을 확인합니다. 9–10에서는 아무것도 남지 않을 수
있습니다: 사이트를 **양쪽에서** 지나간 주행이 아니면 그만큼 좁혀지지 않습니다.

> **방위각은 적지 않습니다.** 두 번 만들어 재보고 뺐습니다 — 정답 위치를 쥐여준
> 계산조차 중앙값 30° 틀렸습니다. 한 도로를 지나는 주행은 안테나를 **좁은 방위 구간에서만**
> 보기 때문이고, 한계가 계산이 아니라 기하에 있다는 뜻입니다. 틀린 방위각은 없는
> 방위각보다 나쁩니다.

**표에서 셀을 고르면 지도가 그 셀로 갑니다.** 가운데 두는 것은 셀 하나가 아니라 **기록과
추정 한 쌍**입니다 — 둘 사이의 간격이 이 화면의 용건이라, 한쪽만 가운데 두면 다른 쪽이
가장자리로 밀립니다. 아래 **Serving cell breakdown** 표의 행도 같은 일을 합니다.

### 3.6c 숫자가 무엇을 기준으로 하는지 본다

범례 머리글과 **Statistics** 화면 모두 **무엇으로 가중했는지**를 적습니다 — `[Sample]`
또는 `[Distance]`, 그리고 dB 평균을 전력 도메인에서 냈으면 `[Sample, linear dB]`.

| 고르는 것 | 뜻 |
|---|---|
| **Weight by: Sample** | 표본 하나가 한 표. 단말이 **겪은 것**에 대한 답 |
| **Weight by: Distance** | 표본이 덮은 **땅**만큼. **도로**가 어떤지에 대한 답 |
| **Mean in: dB as recorded** | 기록된 dB 값의 산술 평균 (관례값) |
| **Mean in: linear power** | 전력으로 바꿔 평균 낸 뒤 되돌린 값 |

> **왜 둘 다 두는가.** 주행 로그는 시계열이라, 신호 대기로 90초 서 있으면 움직이지 않는
> 한 지점에 **표본 90개**가 들어갑니다. 표본 가중은 *"단말이 뭘 겪었나"* 에 정직하고,
> 거리 가중은 *"이 도로가 어떤가"* 에 정직합니다. **둘은 실제로 다른 답을 냅니다** —
> 그래서 어느 쪽인지 말하는 것이 기능의 전부입니다.
>
> 실측(시내 주행): 최악 구간 점유율 **7.07%(표본) → 9.26%(거리)**. A/B 비교 델타는
> RSRP **0.67 → 1.58**, DL_BLER **−0.86 → −2.09** 로 거의 두 배가 됩니다.
>
> **선형 평균은 평균만 바꿉니다.** 백분위는 순서통계량이고 dB→전력 변환은 단조라
> 중앙값은 같은 표본입니다: 평균 **−80.93 → −65.53**, 중앙값 **−80.8 그대로**.
>
> **표본 수는 기준을 바꿔도 그대로입니다.** 백분율 뒤에 측정이 몇 개 있는지가 보여야
> 합니다 — 거리의 90%를 차지하는 구간이 표본 4개인 것과 400개인 것은 다른 이야기입니다.
>
> dB·dBm 단위가 아닌 KPI에는 도메인 선택이 **나타나지 않습니다.** Mbps의 "선형 전력
> 평균"은 아무것도 아닌 것에 대한 산수입니다.
...
내보내기는 **두 자리**에 있습니다 — 주행 전체를 내보내는 툴바, 그리고 **보고 있는 결과를**
내보내는 그 결과 자신의 자리.

**툴바 Export** — 셋입니다. **CSV**(전 KPI 피벗, 한 행이 한 표본) · **GeoJSON**(선택 KPI의
지리 데이터, 플래닝 도구용) · **Report**(인쇄용 세션 리포트 — §3.6-1이 조건 표기를 함께 설명합니다).

**결과에서 바로** — 화면에서 보고 있는 분석을 그 자리에서 파일로 받습니다.

| 어디서 | 무엇이 나오나 |
|---|---|
| Layers 독의 **Area bins** 줄 | 타일 — 중심좌표·표본수·**칠한 값**과 평균/최소/최대, 그리고 GeoJSON은 격자를 자른 그 폭의 폴리곤 |
| Layers 독의 **Serving cell lines** 줄 | 표본마다 선분 하나: 단말이 있던 자리에서 그때 서빙하던 셀까지 |
| Layers 독의 **Estimated cell positions** 줄 | 추정 위치, 그리고 기록까지 잇는 선 |
| **범례**의 `Edit scale` 옆 | 색 구간 — 구간·경계·색·심각도·건수·비율 |
| **Cell locator** 표 머리글 | 그 표 그대로, 기록과의 거리까지 |

> **파일이 자기가 무엇인지 말합니다.** 헤더 위에 `# global_filter: …` 같은 줄이 붙고, **같은
> 내용이 행마다 컬럼으로도** 들어갑니다. 둘 다인 이유는 어느 한쪽이 실제 목적지에서 사라지기
> 때문입니다 — 서른 줄을 다른 시트에 붙여 넣는 순간 서두는 없어지고, GIS는 파일 전체에 붙은
> 정보를 조용히 버리고 도형마다 붙은 것만 보여 줍니다.
>
> **조건을 지키지 않는 결과는 그렇다고 적습니다.** Cell locator는 전역 필터 면제라(confidence가
> 주행 전체로 보정돼 있어 일부만으로는 같은 점수를 주장할 수 없습니다), 조건이 걸린 화면에서
> 받은 파일은 행마다 `not applied — …`라고 말합니다. 빈칸이면 "조건이 아무것도 안 바꿨다"로
> 읽히는데, 그건 다른 이야기입니다.
>
> **레이어를 켜야 그 줄에 링크가 생깁니다.** 지도가 그리지 않는 레이어의 파일은 받는 사람이
> 본 적 없는 그림이기 때문입니다.

**Serving cell lines는 기본이 꺼짐**입니다 — 시내 주행이면 1174개 선이라 경로가 가려집니다.
켜서 부채 모양을 봅니다: 뒤쪽을 가리키면 그 셀이 넘겨야 할 지점을 지나서까지 붙들고 있었다는
뜻이고, 유난히 길면 계획 밖까지 닿고 있다는 뜻입니다.

세 툴바 링크도 결과 링크도 **지금 걸린 전역 필터를 함께 나릅니다.** 리포트는 어느 조건으로
뽑혔는지를 머리 표에 적고, **어느 기준으로 계산됐는지**는 표 바로 아래에 적습니다 — 화면에서
다른 기준을 골라 두었다면 숫자가 다를 수 있고, 리포트가 그렇게 말합니다. 표본 수는 **둘 다**
적습니다(`1174 recorded · 558 under this condition`). 다만 **Problem survey 절은 그 조건을
지키지 않고**, 그 절이 스스로 그렇게 적습니다 — 원인은 이벤트와 검출기 구간에서 나오므로
표본 단위로 좁히면 구간이 쪼개지지 지워지지 않습니다.

> **쓰지 않는 포맷 셋에 이름을 붙여 둡니다** — Excel 바이너리 · MapInfo `.tab` · `.kml`.
> 어려워서가 아니라 **검사할 수 없어서**입니다: `.tab`은 넉 장 중 둘이 바이너리라 우리가
> 열어 볼 수 없고, `.kml`은 Google Earth로 확인한 사람이 여기 없습니다. 그래서 UC23의 파일은
> 자기가 KML이 아니라는 것을 서두에 적습니다.
...
### 4.1 주행 둘이 아니라 빌드 둘을 비교한다 — Cohorts

Compare 탭 맨 위의 **Two drives / Cohorts** 전환입니다. 같은 물음의 **범위**가 다릅니다.

주행 둘의 비교는 빌드 둘의 비교가 아닙니다. 한쪽이 마침 통제된 차선을 만났으면 그 차이는
빌드가 아니라 도로입니다. **Cohorts**는 조건에 맞는 **모든 주행**을 빌드별로 모아 그룹마다
통계 하나를 냅니다.

| 컨트롤 | 하는 일 |
|---|---|
| **Group by** | 무엇으로 묶을지 — 빌드 · 시나리오 · 단말 · 사업자 · 기술 · 장소 |
| **Hold constant** | 비교가 **달라지면 안 되는** 두 번째 차원. 빌드 축의 기본값은 시나리오입니다 |
| 아래 줄 (Search·Device·Operator·Technology·From·To) | 어느 주행이 범위 안인지. 측정 목록과 같은 어휘입니다 |

**세로줄이 그룹의 평균, 속 빈 동그라미가 주행 하나씩입니다.** 그룹의 평균은 어느 주행도
측정하지 않은 숫자입니다 — −6, −6, −14인 세 주행의 평균 −8.7을 "이 그룹은 −8.7"로 읽으면
나쁜 주행 하나가 빌드 회귀로 둔갑합니다. 그래서 멤버를 **함께 그립니다**. 행을 클릭하면 그
그룹의 주행이 표로 펼쳐집니다.

**세로 간격은 시간이 아닙니다.** 행은 각 그룹의 첫 주행 순서로 놓이고 간격은 일정합니다.
그래서 두 평균을 잇는 선은 **직각 꺾은선**입니다 — 대각선은 그 사이에서 값이 매끄럽게
움직였다는 주장인데, 그 사이에서는 아무것도 측정되지 않았습니다. 차트 하단이 그렇게 적습니다.

**Hold constant 를 nothing 으로 두면 판정이 사라집니다.** 델타는 그대로 나오지만 BETTER/WORSE
자리는 비고, 화면이 이유를 적습니다 — 가드 없는 "낫다"는 빌드만큼이나 도로를 재기 때문입니다.
그리고 고정된 차원 때문에 빠진 주행은 **이름과 이유로 목록에 나옵니다**. 개수만 적는 화면은
독자에게 "당신의 답은 불완전하다"고만 말하고 어떻게 불완전한지는 말하지 않습니다.

**첫 그룹에는 판정이 없습니다.** 비교할 앞이 없기 때문이고, 이것은 **NO DATA와 다릅니다** —
NO DATA는 데이터에 관한 주장입니다.

거리 가중은 여기서 **거부됩니다**. 로거가 끊긴 뒤의 표본이 측정되지 않은 구간 전체의 무게를
지므로, 한 주행 안에서는 공정한 거래가 두 주행 사이에서는 로거가 더 자주 끊긴 쪽이 이기는
편향이 됩니다.

주소창의 `by=` 와 `hold=` 가 이 물음을 나릅니다. 링크로 넘기면 **받는 쪽 서버의** 그 빌드
주행들을 가리킵니다 — 세션 id가 아니라 차원값이기 때문입니다.
...
**Build · Scenario · Location 세 칸은 Cohorts 탭이 묶는 축입니다**(§4.1). 설명(Description)에
"1.5.0 고속도로 재시험"이라고 적어도 그것으로는 묶이지 않습니다 — 자유 문장은 GROUP BY 할 수
없고, "1.5.0 고속도로 재시험"과 "고속도로, 1.5.0에서 재시험"은 사람에게는 같은 주행이지만
질의에는 두 코호트입니다. 비워 두면 **(unset)** 이라는 하나의 그룹이 됩니다.

![임포트 화면](./poc-screenshots/24-import-define-kpis.png)

**파일을 여러 개 고를 수 있습니다.** 주행은 파일이 아니라 **폴더**로 나오고, 아래 필드는
**한 번만 채우면** 전부에 적용됩니다. 파일은 차례로 하나씩 들어갑니다.

**들어가는 동안 읽은 행 수가 보이고, Stop 으로 멈출 수 있습니다.**

> 예전에는 *"Importing…"* 만 있었습니다. 숫자도 나갈 길도 없는 그 상태에서 사용자는
> **브라우저의 정지 버튼**에 손을 뻗는데, 그건 응답만 버리고 서버는 끝까지 갑니다.
>
> Stop 은 죽이는 게 아니라 **요청**입니다. 적재 루프가 다음 배치에서 그것을 읽고 멈추며,
> **이미 넣은 것까지 전부 되돌립니다** — 완성된 것처럼 보이는 절반짜리 측정은 측정이
> 없는 것보다 나쁩니다. 이력에는 **CANCELLED** 로 남습니다. FAILED 가 아닙니다 —
> *"당신이 멈췄다"* 와 *"깨졌다"* 는 다른 답입니다.

### 7.1b 측정을 찾는다

툴바 **Find…** 로 이름·빌드 라벨·노트를 검색하고 장비·사업자·기술·기간으로 좁힙니다.

> 좁히기는 **서버가** 합니다. 전부 받아서 대부분 숨기는 방식은 그걸 보여 주는 화면보다
> 훨씬 먼저 무너집니다 — 주간 주행을 하는 팀은 1년이면 수백 건입니다.
>
> 장비·사업자·기술 선택지는 **실제 데이터에서** 나오므로, 아무것도 없는 값을 고를 수
> 없습니다. 결과가 비면 **왜 비었는지** 적습니다 — 걸어 둔 걸 잊은 필터 아래의 빈 목록은
> "측정이 하나도 없다"로 읽힙니다.
...
같은 **Import** 화면에 KPI를 만드는 **세 가지** 방법이 있습니다. **필요한 만큼만 쓰십시오** —
산술식으로 되는 것을 그래프로 그릴 이유는 없습니다.

**Define a measured parameter** — 파일이 들어오기 전에 **측정 컬럼 자체를 정의**합니다. 이름·
표시 이름·단위·분류·소수 자릿수, 그리고 **방향**(클수록 좋은가)까지. 임포트가 알아서 만드는
컬럼은 방향이 `NEUTRAL`이라 자동 스케일이 크기만 칠하므로, 판정 색을 갖게 하려면 이쪽입니다.
...
| **Sample source** | 표본 자체에 기록된 것 — **위도·경도·속도·서빙 PCI**. KPI가 아니라서 예전에는 캔버스에서 아예 닿을 수 없었습니다 |
| **Event source** | 이벤트가 보고된 표본에 표시를 남깁니다. 그 표본은 **1**, 나머지는 **빈 값** — 0이 아닙니다 |
| **Combine** | 여러 입력을 같은 표본에 맞춰 붙입니다. 일부 입력에만 값이 있는 표본도 남습니다 |
| **Correlate** | 한 입력의 값을 **다른 입력의 시점 기준으로** 하나 가져옵니다 — *직전 · 그 순간 · 직후*. 아래 참조 |
| **Expression** | 들어온 열들의 산술. `AS`로 결과 열 이름을 붙입니다 |
| **Filter** | 조건이 참인 표본만 통과 |
| **Classifier** | 조건이 맞는 **첫 상태**로 표본에 이름표를 붙입니다. 상태는 순서대로 1, 2, 3… 값이 됩니다. **기억이 없습니다** — 표본마다 따로 판정합니다 |
| **State machine** | 각 상태를 **얼마나 오래 붙들고 있었는지 밀리초로** 잽니다. 아래 참조 |
| **Output** | KPI가 될 열 하나. 입력에 열이 둘 이상이면 **어느 것인지 골라야 합니다** |

조작: 팔레트 버튼으로 노드를 놓고, 노드를 끌어 옮기고, 노드 **아래쪽 점**에서 다른 노드
**위쪽 점**으로 끌어 연결합니다. 연결선을 클릭하면 끊어집니다.

**노드를 고르면 Preview this node 로 그 노드가 무엇을 내는지 볼 수 있습니다.**
행 수와 표본 몇 줄이 나오고, **아무것도 발행되지 않습니다.**

> 예전 방법은 *KPI 이름을 지어내 발행 → 전 세션에 행을 쓰고 카탈로그에 모두가 보는 항목
> 추가 → 다른 화면에서 확인 → 돌아와 삭제* 였고, 이걸 **추측할 때마다** 했습니다.
> 개인의 불편이 아니라 **여러 사람이 쓰는 상태를 시도할 때마다 변형**하는 문제였습니다.
>
> 행 수는 **노드 전체**를 셉니다. *"3행"* 과 *"41,000행 중 처음 3행"* 은
> *"내 필터가 의도대로 됐나"* 에 대한 다른 답이고, 보이는 몇 줄로는 구별할 수 없습니다.

예: *"움직이는 동안의 RSRP 마진"* 은 **Sample source(SPEED_KMH)** 와 **KPI source(RSRP)** 를
Combine 하고, `SPEED_KMH > 5` 로 Filter 한 뒤 `RSRP + 110` 을 Expression 으로 냅니다.

> **열 이름은 필드 이름 그대로입니다.** Sample source에 `AS`로 다른 이름을 주지 않는 한
> 조건에 쓸 이름은 `SPEED_KMH`이고, 없는 이름을 쓰면 조건이 **거절되면서 쓸 수 있는 이름을
> 목록으로 알려 줍니다** — 조용히 0을 내지 않습니다.

**아래쪽 초록/빨강 띠가 항상 무엇이 잘못됐는지 말해 줍니다.** 그리는 도중에는 그래프가
유효하지 않은 것이 정상이므로, 저장할 때가 아니라 그리는 동안 알려 줍니다.

**실행할 때 값을 묻게 할 수 있습니다.** 조건이나 수식에 `{?이름}` 을 쓰면 띠 아래에 `asked at run time:`
과 그 이름의 입력칸이 생기고, 미리보기와 재계산이 거기 넣은 값으로 돕니다 — 예: `RSRP < {?임계}`.
같은 그래프를 −100과 −110으로 각각 돌려 보는 자리이지, 그래프를 복제하는 자리가 아닙니다.

> **숫자만 받습니다.** 그 밖의 값은 따옴표로 감싸는 것이 아니라 **거절합니다.** 이스케이프는
> "받아들이되 안전하게"인데, 임계값 자리에서 옳은 답은 "받지 않는다"입니다. 값을 비워 두면
> 미리보기는 1로 대신 계산해 **구조만** 검사합니다.

#### 이 일이 나기 **직전에** 무슨 값이었나 — Correlate

**Combine**은 *같은 표본* 위의 값들을 붙입니다. 그것이 Combine이 아는 유일한 관계라서,
*"이 드롭 **직전**의 RSRP"* 는 캔버스로 표현할 방법이 없었습니다. 근본 원인 분석의 질문은
대부분 그 모양입니다.

입력 **둘**을 답니다. 하나가 **primary** — *언제* 답이 나올지를 정합니다. 다른 하나에서 값
하나를 가져옵니다.

| 고르는 것 | 뜻 |
|---|---|
| **previous** | primary의 순간 **직전**의 값 |
| **current** | 그 순간의 값 |
| **next** | **직후**의 값 |
| **previous or current** | 직전 값, 없으면 그 순간 값 |
| **next or current** | 직후 값, 없으면 그 순간 값 |

> **primary가 값이 없는 표본에는 행이 아예 없습니다.** Combine과 정반대이고, 그것이
> 요점입니다 — primary가 곧 *묻고 있는 대상*이기 때문입니다. 원문의 예가 분명합니다:
> RX qual을 primary로 잡으면 **통화 중일 때만** 답이 나오고, RX level을 잡으면 idle 구간까지
> 나옵니다.

**어느 입력이 primary인지는 셀렉트로 고릅니다.** 레퍼런스는 *"가장 왼쪽 입력이 primary"* 라는
배치 규약을 쓰는데, 우리 컴파일러는 노드의 좌표를 읽지 않습니다(같은 그림이 항상 같은 SQL로
컴파일되도록 입력을 노드 번호로 정렬합니다). 그래서 **말로 적는 쪽**을 골랐습니다 — 아무도
강제하지 않는 배치 규약을 배우는 것보다 작습니다.

**Within (ms)** 는 우리가 더한 것입니다. 비워 두면 *"직전"* 이 주행 전체를 거슬러 올라가
**20분 전 값**으로 답할 수 있습니다 — 참이지만 쓸모없습니다. 값을 넣으면 그보다 먼 값은
**가까운 척 보고되지 않고 버려집니다.**

> 예: **Event source(HANDOVER)** 를 primary로, **KPI source(RSRP)** 를 secondary로 걸고
> *previous* 를 고르면, 핸드오버마다 **직전 표본의 RSRP**가 나옵니다. 시드 세션 1의 첫
> 핸드오버(seq 50)에서 −81.6 dBm — 그 순간은 −80.5, 직후는 −79.6입니다.

#### 절차가 몇 밀리초 걸렸나 — State machine

**Classifier**가 *"지금 어느 상태인가"* 에 답한다면, **State machine**은 *"그 상태에 얼마나
있었나"* 에 답합니다. 그것이 이 노드가 존재하는 이유입니다 — 원문이 드는 대표 용도가
**절차 지연 측정**입니다.

사다리로 씁니다. 첫 줄은 **초기 상태**이고 그 조건이 **돌아오는 길**입니다. 그 아래 줄들은
**들어가는 조건**을 위에서부터 순서대로 적습니다.

```
Initial state — return here when:   RECOVERED   RSRP > -100
Then enter when:                    FADED       RSRP < -110
```

저장하면 `FADED`라는 열이 KPI가 되고, 값은 **그 상태에 머문 밀리초**입니다. 값은 **상태에
들어간 표본에** 찍히므로, 지도에서 그 점이 곧 *"여기서 시작됐다"* 입니다.

단위는 고를 수 없습니다 — **`ms`로 고정**됩니다. 지속시간을 `dB`라고 적어 두면 모든 화면이
신호 세기 눈금으로 색칠하기 때문입니다.

**재지 않는 것이 셋 있고, 화면에 적혀 있습니다.**

- 한 표본 안에서 들어갔다 나온 상태
- 끝이 **로거 공백·튄 위치·주행의 끝** 뒤에 오는 상태 — **짧게 자른 값이 아니라 아예 값이
  없습니다.** 재지 않은 시간을 잰 것처럼 적을 수는 없습니다
- 상태를 건너뛰는 전이, 초기 상태 말고 다른 복귀, 그리고 **time trigger**(N ms 안에 조건이
  충족되지 *않으면* 발동하는 전이) — 조건 문법에 부정이 없습니다

> 실제로 이 규칙이 시드에서 걸립니다. 시내 1.4.2 주행의 seq 540 페이드는 **26표본짜리 GPS
> 공백을 건너뜁니다.** 그래서 값이 없습니다. 가드를 빼면 그 자리에 `64000 ms`라는 완벽하게
> 그럴듯한 숫자가 나타나고, 화면의 어떤 것도 그것과 모순되지 않습니다. 같은 모양의 페이드가
> 공백 없는 1.5.0 주행에서는 `71000 ms`로 정상 발행됩니다.
...
- **Drawn as** — **bands**(구간마다 한 색) 또는 **gradient**(구간 사이를 보간). 그라디언트는
  구간에서 만들어지므로 범례와 지도가 서로 다른 말을 할 수 없습니다
...
- 방향이 **NEUTRAL**입니다. 그 컬럼이 "높을수록 좋은지" 알 방법이 없고, 추측하면
  ...
  없습니다**(§8이 편집하는 것은 색상·심각도·경계·그리는 방식입니다) — 원하는 판단은 구간에 **심각도**를
  지정해 표현하십시오
