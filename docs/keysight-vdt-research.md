...
> **확인 수준** 【확인됨】: S8709A 기술개요(`3120-1513`) 4페이지가 구성을 명시합니다 —
> *"The S8709A Virtual Drive Test Toolset is an integrated solution comprising **UXM 5G Network Emulation,
> PROPSIM 5G Channel Emulation, and Nemo Tools**"*. 따라서 **UXM 계열 + PROPSIM 계열 + Nemo**라는
> 조합은 확정입니다.
> 다만 그 안에서 **정확히 `E7515B`인지**(vs. 다른 UXM 모델), **`F8800A`인지 `F8820A`인지**는 문서가
> 모델번호까지 특정하지 않습니다 — 해당 행은 계열 확정 + 모델 【추정】으로 읽으십시오.
...
3. RSRP·SINR·처리량용 **기본 색상 범례** — 보유 자료 전수 스캔 결과 **말뭉치에 없음이 확정**(§11.3.3).
   RSCP·EcNo 두 KPI는 확보. 나머지는 Nemo Analyze 실물 또는 정식 매뉴얼로만 확보 가능하며,
   그 사이의 기본값 근거는 §11.3.4(업계 관행 교차 확인)로 대체합니다.
...
5. S8709A가 쓰는 UXM·PROPSIM의 **정확한 모델번호** — 계열은 확정되었으나(§5.1) 모델 특정은 미확인.
   확보 방법: Keysight 영업·기술 문의.
...
10. ~~Nemo 공식 KPI 색상 범례~~ → RSCP 범례의 구간·hex·통계 구조를 §11.3.3에서 확보.
11. ~~S8709A의 구성 요소~~ → 기술개요 4페이지에서 UXM 5G + PROPSIM 5G + Nemo Tools로 확정 (§5.1).
...
**지도 경로 색상 비닝 — 공식 범례 확보** 【확인됨】

> 2026-08-30 전수 재판독: 보유 PDF 8종에서 200,000픽셀 이상 도판 94개를 추출해 **색상 스와치 수직
> 스택 시그니처**로 전수 스캔했습니다(`tools/legend-extract/measure-legends.py --scan`). 그 결과
> 수치 구간을 가진 KPI 범례는 **RSCP·EcNo 둘뿐**이며, RSRP·SINR·처리량 범례는 이 말뭉치에
> **존재하지 않음**이 확정되었습니다. 아래 값은 모두 같은 스크립트로 재현됩니다.

**(a) RSCP** — 도킹형 `Color Legends` 패널. 동일 범례가 서로 다른 두 도판(p13 Live Google Map,
p15 Area binning)에 나오며 데이터셋만 다릅니다. 아래 값은 **무손실 PNG인 p15**에서 픽셀 순도
100%로 측정한 것입니다. 범례 제목은 `RSCP (dBm) [Time]` — **KPI · 단위 · 가중 기준** 3요소 표기입니다.

| 구간 | 색상 | Hex (무손실 실측) | 샘플 수 | 비율 |
|---|---|---|---|---|
| `>= -80` | 녹색 | **`#009300`** | 6,749 | 63.66% |
| `< -80 and >= -90` | 노랑 | **`#FFFF00`** | 2,796 | 26.37% |
| `< -90 and >= -100` | 주황 | **`#FF6820`** | 871 | 8.22% |
| `< -100` | 빨강 | **`#FF0000`** | 186 | 1.75% |

> **정정**: 이전 판은 같은 패널의 **JPEG 사본**(p13)에서 값을 읽어 `#009200`·`#FF681F`·`#FE0000`으로
> 적었습니다. 셋 다 압축 아티팩트였고, 무손실 원본은 위 표가 맞습니다. 시드 상수도 함께 정정했습니다.

**(b) EcNo** — 구세대 부동(floating) 지도 범례. **두 번째로 확보한 KPI 구간**입니다.
...
1. **RSCP 경계는 -80 / -90 / -100 dBm**, 4구간입니다. 앞서 "업계 관용"이라고 적었던 임계값이 실제
   Nemo 기본값과 일치합니다.
2. **EcNo는 -5 / -8 / -10 dB**로, 경계 간격이 균일하지 않습니다(3 dB, 2 dB). 즉 Nemo의 구간은
   등간격 규칙이 아니라 **KPI별 공학적 판단**으로 정해집니다 — 새 KPI에 등간격을 기계적으로
   적용하면 안 된다는 뜻입니다.
3. **4구간·녹황주적 램프는 KPI를 가리지 않고 동일**합니다. 서로 다른 두 세대의 UI, 서로 다른 두 KPI가
   같은 램프를 씁니다. 이것이 이 도구 시각 언어의 상수입니다.
4. **범례가 곧 통계입니다.** 도킹형 패널은 각 구간의 **샘플 수와 구성비**를 함께 보여줍니다.
   "경로의 63.66%가 -80 dBm 이상"을 한눈에 읽게 하는 것이 이 패널의 진짜 기능입니다.
   POC의 범례는 반드시 이 통계 열을 포함해야 합니다.
5. **초록은 세대에 따라 다릅니다** — 구세대 부동 범례는 순색 `#00FF00` 계열, 현행 도킹 패널은
   짙은 `#009300`입니다. 우리는 현행 무손실 실측값을 따릅니다.

> 구간 표기법은 두 가지가 관측됩니다 — 도킹 패널은 `< -80 and >= -90`(상한 먼저), 부동 범례는
> `>= -8 and < -5`(하한 먼저). 어느 쪽이든 **경계 포함 관계를 명시**한다는 점이 공통입니다.
> 우리 구현은 도킹 패널 표기를 따릅니다.

지도 배경은 **LiveMaps** 목록에서 선택합니다 — Google Streets / Hybrid / Satellite / Physical / Grayscale,
그리고 **OpenStreetMap (osm)** 【확인됨】. 별도로 MapInfo `.TAB` / `.GST` 파일을 `Loaded MapX Maps`로
올릴 수 있습니다.

**다른 KPI의 구간** 【미확인 — 말뭉치 소진 확정】: RSRP·SS-SINR·처리량용 기본 범례는 보유 자료에
**없음이 전수 스캔으로 확정**되었습니다(추가 탐색 불필요). 3D Visualizer 도판은 `SS-RSRP`를 표시하면서도
`Legends` 패널이 접힌 상태라 구간값을 노출하지 않으며, 색상 램프가 위와 동일한 녹→황→주→적임만
확인됩니다. 따라서 이 세 KPI의 기본 구간은 **다른 근거**(업계 관행 교차 확인 및 규격 유도)로 세워야 하며,
그 조사 결과는 §11.3.4에 별도로 기록합니다.
...
#### 11.3.4 RSRP·SINR·RSRQ·처리량 구간 — 외부 교차 확인

§11.3.3에서 Keysight 말뭉치에 이 네 KPI의 범례가 **없음이 확정**되었으므로, 기본값의 근거를
독립 출처 교차 확인으로 세웠습니다. 아래 두 출처는 **제가 직접 원문을 받아 검증**했습니다
(하위 에이전트 검증 단계는 계정 세션 한도로 중단되어, 검증되지 않은 나머지 주장은 §11.3.5에
별도로 격리했습니다 — 인용 금지).

**출처 A — Android Open Source Project** 【확인됨: 원문 직접 확인】
`platform/frameworks/base` · `telephony/java/android/telephony/CellSignalStrengthNr.java`
(`android.googlesource.com` 원문 취득, base64 디코드 후 판독). 주석에 "Lifted from Default
carrier configs"라고 적힌 **기본 캐리어 설정값**입니다.

| KPI | POOR | MODERATE | GOOD | GREAT | 규격상 보고 범위(주석) |
|---|---|---|---|---|---|
| SS-RSRP | -110 | -90 | -80 | -65 | -156 ~ -31 dBm |
| SS-RSRQ | -31 | -19 | -7 | 6 | -43 ~ 20 dB |
| SS-SINR | -5 | 5 | 15 | 30 | -23 ~ 40 dB |

판정은 `measure >= thresholds[n]` — **하한 포함** 비교로, Nemo 범례의 `>=` 규약과 동일합니다.

**출처 B — AZQ(Azenqos) QGIS 플러그인 기본 테마** 【확인됨: 원문 직접 확인】
`Azenqos/default_theme.xml`(raw.githubusercontent.com에서 취득, XML 파싱). 실제 **드라이브 테스트
도구**의 출하 기본 범례라는 점에서 도메인 적합도가 가장 높습니다.

| KPI(elementID) | 구간 경계 |
|---|---|
| `nr_servingbeam_ss_rsrp` | -150 / -124 / -116 / **-110** / **-100** / -95 / **-90** / **-80** / 0 |
| `nr_servingbeam_ss_sinr` | -10 / **0** / **5** / 10 / **15** / 20 / 50 |
| `nr_servingbeam_ss_rsrq` | -20 / -14 / -11 / -9 / -6 |
| `nr_mac_dl_tp_mbps` | 0 / **50** / **100** / 200 / **300** / 400 / 600 / 800 / 1500 |

> 주의: AZQ의 색상 램프는 녹-적 계열이 아니라 **무지개 계열**(마젠타→주황→노랑→녹→청→파랑)이며
> 파랑이 최상입니다. 따라서 AZQ는 **구간 경계의 근거이지 색상의 근거가 아닙니다**. 색상은
> §11.3.3의 Nemo 실측값을 따릅니다.
> 또한 `nr_mac_ul_tp_mbps`가 DL과 **완전히 동일한 사다리**를 쓰는데, 같은 파일의
> `lte_l1_ul_throughput_all_carriers_mbps`는 5/10/15/20/30/42/60이라는 별도 UL 척도를 씁니다.
> NR UL 쪽은 복제로 보이므로 채택하지 않습니다.

**수렴 판정과 우리 기본값**

| KPI | 우리 기본값 | 출처 A | 출처 B | 판정 |
|---|---|---|---|---|
| RSRP | **-80 / -90 / -100** | -80, -90 일치 | -80, -90, -100 모두 일치 | **교차 확인됨** — 3개 경계 전부 최소 1개 독립 출처, `-80`·`-90`은 3개 출처 전부(Nemo RSCP 포함) |
| SS-SINR | **0 / 5 / 15** | 5, 15 일치 | 0, 5, 15 모두 일치 | **교차 확인됨** — 변경 불필요 |
| DL 처리량 | ~~20~~ → **50 / 100 / 300** | (해당 없음) | 50, 100, 300 모두 일치 | **정정함** — 기존 하한 20은 어떤 출처에도 없었음 |
| RSRQ | -10 / -15 / -20 | -7 / -19 / -31 | -6 / -9 / -11 / -14 / -20 | **자체 기본값 유지** 【추정】 — 아래 참조 |

RSRQ만 출처 간 편차가 큽니다. AOSP는 규격 보고 범위 전체(-43~20)에 걸친 넓은 척도이고, AZQ는
실측 분포에 맞춘 좁은 척도(-20~-6)입니다. 우리 값은 두 척도 사이의 **반올림된 중간값**으로,
각 경계가 두 출처 중 하나에서 1~3 dB 이내에 있습니다(-10↔-9, -15↔-14, -20↔-20). 1~3 dB 차이를
위해 시드를 흔들 근거가 약하고, Nemo 자신의 RSCP가 -80/-90/-100이라는 **반올림 관행**을 쓰므로
가독성 면에서도 현행 유지가 낫다고 판단했습니다. 다만 네 KPI 중 **근거가 가장 약한 값**임을 명시합니다.

> **부수 발견(데이터 현실성)**: 처리량 범례의 최상위 구간(`>= 300 Mbps`)이 시드 데이터에서
> **0.00%**입니다. n78 100 MHz 4x4 구성이면 실제 드라이브 테스트는 수백 Mbps~1 Gbps에 달하는데
> 현재 생성기의 최대값은 약 235 Mbps입니다. 범례 구간의 문제가 아니라 **생성기 보정의 문제**이며,
> 별도 항목으로 기록합니다.

#### 11.3.5 미검증 주장 — 인용 금지

아래는 조사 단계에서 수집되었으나 **적대적 검증이 실행되지 못한** 항목입니다(계정 세션 한도로
검증 에이전트 21개 전부 중단). 반증된 것이 아니라 **확인되지 않은** 것이므로, 검증 전까지
근거로 인용하지 마십시오.

| 주장 | 출처(주장된) | 미검증 사유 |
|---|---|---|
| TEMS Investigation 22.3의 CPICH RSCP 3구간(-135/-100/-85), GSM RxLev 5구간 | Infovista 기술설명서 p.52 스크린샷 | 도판 판독(inferred-from-figure), 원문 재확인 안 됨 |
| Accuver XCAL DL 처리량 5구간(1/100/300/3000) | XCAL Common User Guide (2012) | 동상. 상단 구간 값이 패널에 잘림 |
| Anritsu 커버리지 매핑 RSSI 5구간, 5GNR SS-RSRP(-85/-100/-115) | Anritsu 측정 가이드·Site Master 도움말 | 동상. RSSI는 RSRP와 다른 양 |
| TEMS Discovery의 `From(>=) / To(<)` 반개구간 규약 | TEMS Discovery 10.0 사용자 가이드 | 규약 자체는 우리 표기와 일치하나 재확인 필요 |
| 개인 라이브러리(hugoh/cellular-signal)의 RSRP/SINR/RSRQ 등급 | GitHub 개인 저장소 | 소스 에이전트 스스로 "0-star 개인 라이브러리"로 신뢰도 낮음 표기 |

**교차 확인에서 확인된 사실 하나**: 경쟁 드라이브 테스트 벤더(Infovista·R&S·Accuver·Anritsu·VIAVI)
중 **5G NR SS-RSRP/SS-SINR/NR RSRQ의 기본 색상 구간을 공개 문서로 발행하는 곳은 없습니다**.
따라서 이 값들은 업계 어디서도 "공식 기본값"으로 확정되어 있지 않으며, 우리가 근거를 명시한
기본값을 두는 것이 현실적으로 가능한 최선입니다.
