...
3. RSRP·SINR·처리량용 **기본 색상 범례** — RSCP 범례는 확보했으나(§11.3.3) 나머지 KPI는 미확보.
   확보 방법: Nemo Analyze 실물.
...
Nemo Analyze 공식 기술개요(`5992-2005EN`) Figure 6 "Live Google Map"에서 **실제 `Color Legends` 패널을
판독**했습니다. 범례 제목은 `RSCP (dBm) [Time]`이며 구조는 **구간 · 색상 · 샘플 수 · 구성비(%)** 4열입니다.

| 구간 | 색상 | Hex (실측) | 샘플 수 | 비율 |
|---|---|---|---|---|
| `>= -80` | 녹색 | **`#009200`** | 5,985 | 72.49% |
| `< -80 and >= -90` | 노랑 | **`#FFFF00`** | 1,900 | 23.01% |
| `< -90 and >= -100` | 주황 | **`#FF681F`** | 346 | 4.19% |
| `< -100` | 빨강 | **`#FE0000`** | 25 | 0.30% |

읽어낼 점이 세 가지 있습니다.

1. **구간 경계는 -80 / -90 / -100 dBm**, 4구간입니다. 앞서 "업계 관용"이라고 적었던 임계값이 실제
   Nemo 기본값과 일치합니다.
2. **색상은 순색 계열**입니다 — 순수 노랑 `#FFFF00`, 순수 빨강 `#FE0000`. Nemo의 데이터 색상 관행(§11.3.2)과 같습니다.
3. **범례가 곧 통계입니다.** 단순 색상 키가 아니라 각 구간의 **샘플 수와 구성비**를 함께 보여줍니다.
   "경로의 72.49%가 -80 dBm 이상"을 한눈에 읽게 하는 것이 이 패널의 진짜 기능입니다.
   POC의 범례는 반드시 이 통계 열을 포함해야 합니다.

> 구간 표기법도 그대로 쓸 만합니다 — `< -80 and >= -90` 처럼 **경계 포함 관계를 명시**합니다.
...
**다른 KPI의 구간** 【미확인】: RSRP·SINR·처리량용 기본 범례는 확보하지 못했습니다. 다만 위 RSCP 범례의
**구조**(4구간, 순색, 통계 동반)가 제품 전반의 관행으로 보이며, RSRP도 같은 -80/-90/-100 경계를 쓰는 것이
업계 통례입니다 【추정】.
