...
- 거리 빈 크기를 캐리어 주파수에서 **40λ**로 계산하는 옵션 — 거리 비닝 자체는 구현됐으나 **그것이 Lee's criteria는 아닙니다**(§1.2). 변환 함수는 `FieldToLabService.centreFreqMhz`에 이미 있음
- 여러 세션 병합·컬렉션

### 3.3 P2 — 확장

- ~~이웃 셀 측정 스키마 추가~~ → **완료** (`V7__sample_neighbour.sql`). 예고대로 pilot pollution과 overspilling 단서가 함께 열렸고, neighbor list만 남았습니다
...
| 랩 런 **생성** UI + 실행기 | *2026-09-03 분할.* **시작은 배선돼 있습니다** — `client.ts`의 `startRun`을 `BringUpPanel`의 버튼이 부릅니다. 남은 것은 런을 **만드는** UI(채널 모델·셀 구성·UE 프로파일·DU를 고르는 폼)와 실제 장비를 구동하는 실행기이고, 뒤쪽은 의도적 보류입니다(`architecture-and-scale.md` §2.6) |
| ~~임계값 편집 UI (NEW-03)~~ | **2026-08-30 완료** — §5.3 참조 |
| 빌드 간 추이(trend) 뷰 | 캠페인 모델이 이미 지원. Compare가 2개 세션을 넘어 N개 런 추이로 확장되는 형태 |
| ~~세션 선택기 검색~~ | **2026-09-01(⑧) 완료** — `SessionFilter` + `GET /api/sessions/facets`. 그룹핑(폴더)은 여전히 보류이고, P4-3에서 **차원값으로 묶는 쪽**을 골랐습니다 |
| ~~셀별 바 차트 (FR-18)~~ | **완료** — `Cells` 탭. 히스토그램은 CDF·분포 뷰가 대신하고 있어 별도 항목으로 남기지 않습니다 |
| ~~RACH 상세 그리드 (FR-33)~~ | **완료** — `run_rach`(V4) + `BringUpPanel`의 RACH 표. 남은 것은 `maximum preamble`·`preamble response` 2필드뿐이고 `ui-gap-vs-reference.md` §6(b)에 작음으로 있습니다 |
