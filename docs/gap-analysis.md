...
| Advanced data filtering and global filters | ✅ | **2026-09-02(P3-2) 해소.** `kpi:NAME:OP:VALUE` · `cell:PCI` · `notevent:TYPE`을 창 맨 위 한 줄에 걸면 **17개 분석 엔드포인트**가 그 조건으로 답합니다. 지키지 못하는 12개는 `GET /api/global-filter/coverage`에 **이유와 함께** 실려 화면(Reach 17 of 29 — 2026-09-05 기준)에서 읽힙니다 — 조용히 무시하는 화면이 없다는 것이 이 항목의 요구입니다. **2026-09-03(P4-3)**: 조건의 키가 `(session_id, seq)` 쌍이라 여러 주행에도 걸립니다. **2026-09-05(F2·F3)**: 자기 사유에 "배선이 안 됐을 뿐"이라고 적혀 있던 둘이 배선됐습니다 — `/compare`(두 주행을 **각자 자신에 대해** 좁혀 비교, 시드 주행에서 RSRP 판정이 뒤집힙니다)와 `/coverage-issues`(세 검출기 전부, **구간을 묶기 전에** 좁히므로 한 구간이 둘로 갈라질 수 있고 개수가 늘 수 있습니다). 남은 "아직 안 함"은 `spatial-diff`와 `problem-survey` 둘 |
...
| **Exporting workbooks** (p223 — 전사본에 메뉴까지 있습니다: `File \| Workbook to File` → PDF·Word·PowerPoint, `File \| Page Image to clipboard \| Workbook images to file` → 페이지별 이미지, `Export ratio` % 또는 640×480) | ◐ **2026-09-05 신규.** 구성한 워크북이 **문서 한 장(인쇄용 HTML+인라인 SVG)** 과 **페인별 SVG**로 나갑니다 — 화면이 쓰는 것과 **같은 기하 모듈**이 그리므로 두 그림이 어긋날 수 없고, 검사기가 Node로 그 모듈을 불러 브라우저가 그린 `d`와 문자 단위로 대조합니다. 파일은 조건(서버 문장 그대로)·측정·저장 여부·빠진 것을 **서두와 페인 양쪽에** 싣습니다. 남은 것: **PDF·Word·PowerPoint 자체**(브라우저 인쇄가 PDF를 만들고, 파일이 자기가 그 셋이 아니라고 적습니다)와 **래스터 이미지**(OSM 타일에 `crossOrigin`이 없어 캔버스가 오염되고, PNG는 매직 바이트 너머를 검사할 수 없습니다), **페이지 단위**(우리 스키마에 페이지가 없어 페인으로 냅니다), 그리고 **`Export ratio`·페이지 `Fixed size`** — 우리 그림은 페인 종류마다 고정 치수(차트 1000×200 · 지도 1000×560)이고 고르는 자리가 없습니다 |
