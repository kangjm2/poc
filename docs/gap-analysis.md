...
| 항목 | 사유 |
|---|---|
| 사용자 정의/파생 KPI 생성 (P0) | 임계값 없는 KPI의 지도·범례 렌더링 경로(중립 단일 빈 or 분위수 자동 스케일) 설계가 선행되어야 안전. 임포트의 "미지 컬럼 → KPI 자동 생성"도 이것에 종속 |
...
| 임계값 편집 UI (NEW-03) | PUT 엔드포인트 존재. 범례 패널 내 편집기로 붙이는 것이 자연스러우나 이번 라운드 범위 초과 |
