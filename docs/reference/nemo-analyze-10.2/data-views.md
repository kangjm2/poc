...
> 라이브러리가 없습니다.** *(2026-09-04: 툴바에 CSV·GeoJSON·리포트 셋이 있고, 그 위에 분석 결과 넷이 더 있습니다 — 타일 · 범례 · 추정 셀 위치 · 서빙 셀 선.)*
> **2026-09-05 — 워크북 자신의 내보내기가 생겼습니다.** 워크북 헤더에서 `Export document`(전체를
> 인쇄용 HTML 한 장으로)와 `pane N`(페인마다 홀로 열리는 SVG). 레퍼런스가 p223에서 내는
> **PDF·MS Word·MS PowerPoint는 내지 않고, 파일이 스스로 그렇게 적습니다** — PDF는 브라우저 인쇄가
> 만듭니다(리포트 HTML과 같은 답, 같은 이유). 그리고 내보내는 단위는 **페인**입니다: 레퍼런스의
> 단위는 페이지인데(p220) 우리 스키마에 페이지가 없어서, 없는 것을 있는 말로 부르지 않습니다.
> 리포트는 여전히 워크북이 아니라 **세션** 단위입니다.
...
*(2026-09-05: 우리 워크북 내보내기는 이 자리가 아니라 위 §6입니다 — 문서 한 장과 페인별 SVG이고,
템플릿 디자이너와는 무관합니다.)*
