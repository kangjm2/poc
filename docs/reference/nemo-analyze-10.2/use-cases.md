...
*2026-09-04 보탬 — 이 UC에서 남아 있던 두 항이 들어왔습니다.* `Create Global Filter From Cell ID`가
**진입점을 얻었습니다**(지도에서 셀 사이트 우클릭 → 버튼 하나짜리 팝업이 묻습니다. 우클릭 자체로
걸지 않는 이유는 필터가 화면 전체를 다시 좁히기 때문입니다 — 붐비는 지도에서 사고로 발동할
조작입니다). 그리고 `Exclude Events`(p94)가 문법의 **`notevent:TYPE`**이 됐습니다. **±5표본 창은
우리가 더한 것**이고 필터 설명이 그 숫자를 말합니다 — 매뉴얼이 지우는 것은 실패한 *호*인데
우리에겐 호 경계가 없고, 표본 하나를 빼는 것은 어떤 백분위도 바꾸지 않습니다. 실측으로 1200 →
1178표본이고 **P05가 −115.5 → −114.1**입니다. 이벤트를 표본으로 옮기는 규칙은 `EventOnSample`
한 곳에 있고 셋이 바인딩합니다(이벤트 목록 · 전역 필터 · Problem Survey).
...
*2026-09-04 보탬.* p149의 **셀 사이트 우클릭**이 생겼습니다(→ UC5의 `Create Global Filter From
Cell ID`). 만들면서 **기능보다 값진 결함 하나**가 나왔습니다 — 셀 사이트가 **자기 중심점에서
클릭이 안 됐습니다.** 모든 레이어가 Leaflet 기본 오버레이 페인을 공유해 쌓임이 DOM 순서(=마지막에
돈 이펙트)였고, 경로는 KPI·색·격리가 바뀔 때마다 다시 그려져 마커 위에 얹혔습니다. 남은 자리는
방위각 스포크가 삼켰고요 — 핸들러도 없는 장식인데 사이트 중심에서 시작합니다. 사이트 전용
페인(zIndex 450) + 스포크 비대화형으로 고쳤고, **줄곧 있던 PCI 툴팁도 그제서야 닿습니다.**
...
*2026-09-04 정정 — 위 문단은 두 곳에서 낡았습니다.* (1) **상태를 세는 곳은
`../../use-case-coverage.md` 한 곳**이고 거기서 UC15는 ◐입니다. 여기 ✅는 그 표가 생기기 전의
표기입니다. (2) "없는 것"의 **빈별 `Statistic` 전환은 2026-09-02에 생겼고**(`BinStatistic`이
평균·최소·최대를 내고 지도 머리글이 어느 것인지 적습니다), **레이어 내보내기는 2026-09-04에
생겼습니다**(Layers 독의 `Area bins` 줄 → CSV·GeoJSON, 칠한 통계를 결과에서 읽어 파일에 적음).
지금 없는 것은 **빈 값 인쇄(`Print value`)**, **표준편차·분산·최빈**, 그리고 원문이 제시하는
**Excel 바이너리와 MapInfo `.tab`** 포맷입니다.
...
한쪽만 지나간 타일은 0이 아니라 null. 레퍼런스와 같은 "빈별 평균 → 빼기"입니다.

*2026-09-03 정정 — 이 문단은 두 가지를 반대 방향으로 틀리게 적고 있었습니다.* 첫째, **"가중 기준
선택도 같습니다"는 거짓입니다**: 위 7단계의 `Choose Statistics Type`(`Time` / `Distance` /
`Sample`)에 해당하는 것이 우리에겐 없고, `SpatialDiffService`는 맨 `avg(k.value)`로 비닝하며 basis
인자를 받지 않습니다. 둘째, **"그룹 평균이 없다"도 이제 거짓입니다** — P3-4에서 반대편이 측정
그룹이 됐습니다(가까운 쪽은 아직 한 주행이고 그것이 남은 격차입니다).

근거 계층의 맨 위에 있는 문서에 거짓 문장이 있는 것이 가장 나쁩니다. 다음 사람이 여기를 사실로
...
*2026-09-04 — 그 세 조각이 만들어졌고, 이 UC는 다시 ✅입니다.* 위 목록이 이름 붙인 순서 그대로입니다:
행 선택(`CellBarChart.tsx`의 `BreakdownTable`, 행 전체가 누를 수 있는 자리) → PCI(`App.tsx`의
`focusPci`) → 지도 이동(`RouteMap.tsx`, `fitBounds`). **막고 있던 것은 셋 중 어느 것도 아니라
"Cells 탭에 지도가 없다"였고**, 그래서 `onPickCell`이 넘겨지지 않은 채로 있었던 것입니다 — 그
지도가 UC21의 추정 위치가 앉을 자리이기도 해서 둘이 한 번에 열렸습니다. 프레임이 가운데 두는 것은
셀 하나가 아니라 **기록–추정 한 쌍**입니다(둘은 수백 m 떨어져 있어 한쪽을 중심에 두면 다른 쪽이
가장자리로 밀립니다). 검사(`verify-scenarios` S28)는 **속성과 좌표를 따로** 겁니다 —
`data-focus-pci`는 행이 지도까지 닿았는지를, 화면 좌표는 지도가 그것을 받아 실제로 움직였는지를
말합니다. 밖에서 보면 같아 보이지만 고칠 곳이 다른 두 실패입니다.
...
*2026-09-04 — 만들었습니다(◐).* 한때 이 기능을 **"정답을 알고 있으니 데모"**로 미뤘는데 사정이
거꾸로였고, 그렇게 말하는 것이 위 그림입니다 — p175는 실제와 추정을 **한 지도에** 그립니다.
기록을 갖고 있는 것이 이 기능의 **사용 조건**입니다: 셀 DB는 이설·급전 교체·좌표 오기입으로 낡고,
추정과 기록이 300 m 어긋나면 의심할 것은 기록입니다. 드라이브는 그 기록에 대해 누구나 정기적으로
모으는 **유일한 독립 증거**입니다.

- **추정기는 측정으로 골랐고 교과서 답이 졌습니다.** 경로손실 피팅(RSRP → 거리, 삼변측량)이 모든
  분위에서 더 나빴습니다 — 중앙값 193 m 대 67 m, 3.7 km짜리 이상치 하나. 이유는 RSRP가 거리만의
  함수가 아니어서입니다: 섹터 로브가 방위에 따라 최대 20 dB를 얹고, 한 도로를 지나는 주행은 그
  방위를 좁은 구간에서만 봅니다. 채택한 것은 **최강 표본 8 dB 이내의 전력가중 센트로이드**이고
  네 주행 22셀에서 중앙값 54 m입니다.
- **방위각은 보고하지 않습니다.** 인용문이 위치와 함께 약속하는 값이지만, 두 번 만들어 재보고
  뺐습니다 — 첫 시도는 147–179° 틀렸고, **정답 위치를 쥐여준 로브 피팅도 중앙값 30°**였습니다.
  한계가 추정기가 아니라 기하에 있다는 뜻이고, 틀린 방위각은 없는 방위각보다 나쁩니다.
- **confidence는 실제 오차와 상관이 있는 것으로 만들었습니다.** 처음 쓴 "퍼짐 × 표본수"는 상관이
  −0.27 / −0.02라 숫자가 아니라 장식이었습니다. 지금은 **최강표본 세기**(−0.83)와 **도로
  근접도**(+0.70)의 곱입니다. 검사는 공식이 아니라 **주장**을 겁니다 — 레퍼런스가 자기 점수에
  붙인 "6 이상이면 <100 m"(`verify-scenarios` S28). 실제로 그 주장이 한 번 깨졌습니다: 보정에
  쓰지 않은 네 번째 주행에서 confidence 6짜리가 198 m로 나왔고, 근접 창을 20 dB → 10 dB로 좁혀
  고쳤습니다.
- **빠진 단계 셋.** 위의 방위각, 화면에서 고르는 `Carrier number`(서버는 `arfcn`을 받지만
  `DataSeeder`의 셀이 전부 ARFCN `633984` 하나라 UC1의 채널 번호 필터와 같은 처지입니다),
  그리고 레이어 내보내기(`.kml` · MapInfo `.tab` · 그리드 `Export Data To`). `Minimum accuracy
  score`는 화면에 있고 원문의 "9–10은 전부 걸러질 수 있다"는 경고까지 화면이 말합니다.
...
입력의 어떤 조각도 SQL로 복사되지 않습니다. 없는 것: `<Previous value>` 비교.

*2026-09-04 정정 — 이 자리에 `{?변수}`도 적혀 있었고 이제 있습니다.* 한 번 "안 만든다"고 적었던
항목이고 그 논거가 틀렸습니다: 근거는 "발행된 그래프는 이름 하나에 정의 하나여야 한다"였는데,
레퍼런스의 변수는 **정의를 바꾸는 것이 아니라 실행 때 값을 묻는 것**입니다(p398). 바인딩은
**숫자만** 받고 못 알아들을 값은 이스케이프가 아니라 **거절**합니다 — 이스케이프는 "받아들이되
안전하게"이고, 여기서 옳은 답은 "받지 않는다"입니다(`KpiGraph.bind` · 테스트가 `0 OR 1=1`과
`-100; DROP TABLE`을 겁니다).
...
## 2026-09-04 — 구현이 따라온 곳

전사는 여전히 그대로입니다. 위 절이 **코드를 열어 판정을 내린** 자리라면, 여기는 그 판정이
가리킨 것을 **만든** 자리입니다 — 다섯 곳 전부 위 표가 이름 붙인 격차 그대로입니다.

| 자리 | 무엇이 생겼나 | 어디 |
|---|---|---|
| UC5 · UC14 | `Create Global Filter From Cell ID`의 **진입점**(지도 우클릭)과 `Exclude Events`(`notevent:TYPE`) | `RouteMap.tsx` · `GlobalFilter.java` · `EventOnSample.java` |
| UC18 | **셀 행 → 지도 줌.** 막고 있던 것은 위 표가 든 셋이 아니라 "Cells 탭에 지도가 없다"였습니다 | `CellBarChart.tsx` · `App.tsx`의 `focusPci` · `RouteMap.tsx` |
| UC21 | **Cell locator.** 방위각은 재보고 뺐습니다 — 정답 위치를 줘도 중앙값 30° | `CellLocatorService.java` · `CellLocatorPanel.tsx` |
| UC26 | `{?변수}` — "안 만든다"고 적었던 논거가 틀렸습니다 | `KpiGraph.bind` · `KpiWorkbench.tsx` |
| UC6(§2 도크) | 차트 **구간 줌**과 **임계선**. 이 UC가 빠뜨린 좌/우 축은 여전히 만들지 않기로 | `TimeSeriesChart.tsx` |

**한 가지를 배웠습니다.** UC18과 UC21은 서로 다른 격차로 적혀 있었는데 **같은 하나에**
막혀 있었습니다 — Cells 탭에 지도가 없어서 행 클릭은 갈 곳이 없었고 추정 위치는 앉을 자리가
없었습니다. 단계 단위로 적어 두지 않았다면 그 둘이 한 원인이라는 것도 보이지 않았을 것입니다.
