...
후 중복 제거·반증 재개 차단(§4 대조)·완결성 크리틱을 거친 랭킹. 28건 후보 → 19건 확정 + 방법론 트랙 9건.
...
**왜 지금** — gap §1.2·§1.3에서 이웃 셀 데이터 부재가 분석 3종을 원천 봉쇄(P2 명시 이연). AZQ 임포터 항목이 데이터 공급을, 이 항목이 스키마·판정 기준을 맡는 상보 관계 — 기준 없이 스키마부터 만들면 되돌리게 됨.
