...
public class ReportService {
    ...
    public String render(long sessionId, String filterSpec) {
        ...
        // Read off the result rather than restated here, so the printed basis is the one the
        // query actually used and cannot drift from it.
        ...
        for (KpiDefinition def : defs) {
            ...
            if (basisLabel == null && st.count() > 0) basisLabel = st.basisLabel();
        ...
        // Beside the numbers, because a report is read furthest from the screen that made it,
        // and a table of means with no stated basis is indistinguishable from one computed
        // the other way. This report always asks for the default; the screen lets the reader
        // pick distance weighting or a linear-power mean, so the two CAN differ.
        if (basisLabel != null) {
            b.append("<p class=\"foot\">Means and percentiles ").append(esc(basisLabel))
             .append(". The screen offers other bases; set the same one there to compare.</p>");
        }
        ...
        b.append("<p class=\"foot\">Generated by VDT Analyzer from session ")
         ...
         + "on-screen analysis uses, on the basis named above \u2014 so this report agrees "
         + "with the screen whenever the screen is read on that same basis.</p>");
